...
class AV_PAIRS:
    ...
    def getData(self):
        """
        Genera los AV_PAIRS en el orden: 02, 01, 04, 03, 05, 07, 06, 08, 0a, 09, 00
        """
        tInfo = b''
        
        # Orden específico requerido
        field_order = [
            0x0002,  # NTLMSSP_AV_DOMAINNAME
            0x0001,  # NTLMSSP_AV_HOSTNAME
            0x0004,  # NTLMSSP_AV_DNS_DOMAINNAME
            0x0003,  # NTLMSSP_AV_DNS_HOSTNAME
            0x0005,  # NTLMSSP_AV_DNS_TREENAME
            0x0007,  # NTLMSSP_AV_TIME
            0x0006,  # NTLMSSP_AV_FLAGS
            0x0008,  # NTLMSSP_AV_SINGLE_HOST (Restrictions)
            0x000a,  # NTLMSSP_AV_CHANNEL_BINDINGS
            0x0009,  # NTLMSSP_AV_TARGET_NAME
        ]
        
        # Procesar campos en el orden especificado (EXCEPTO EOL)
        for field_type in field_order:
            if field_type in self.fields and field_type != NTLMSSP_AV_EOL:
                length, content = self.fields[field_type]
                tInfo += struct.pack('<HH', field_type, length) + content
        
        # Agregar campos adicionales no contemplados (si existen, excepto EOL)
        for field_type in self.fields.keys():
            if field_type not in field_order and field_type != NTLMSSP_AV_EOL:
                length, content = self.fields[field_type]
                tInfo += struct.pack('<HH', field_type, length) + content
        
        # CRÍTICO: EOL SIEMPRE al final con longitud 0
        # No importa si está en self.fields o no, SIEMPRE debe estar
        tInfo += struct.pack('<HH', NTLMSSP_AV_EOL, 0)
...
def computeResponseNTLMv2(flags, serverChallenge, clientChallenge, serverName, domain, user, password, lmhash='',
    ...
    # Parsear los AV_PAIRS del servidor
    ...
    if TEST_CASE is False:
        ...
        # ==================== CAMPOS ADICIONALES ====================
        # Agregar TARGET_NAME (0x09) basado en HOSTNAME
        av_pairs[NTLMSSP_AV_TARGET_NAME] = 'cifs/'.encode('utf-16le') + av_pairs[NTLMSSP_AV_DNS_HOSTNAME][1]

        # Agregar FLAGS (0x06)
        av_pairs[NTLMSSP_AV_FLAGS] = struct.pack('<L', 0x00000000)
        
        # Agregar SINGLE_HOST/Restrictions (0x08)
        restrictions_content = bytes.fromhex('30000000000000000100000000200000bb0e21635325a81e7e6c01b72a87cbf45dbf8e9d86f99f1d2fa57f0827cc753a')
        av_pairs[NTLMSSP_AV_RESTRICTIONS] = restrictions_content
        
        # Agregar CHANNEL_BINDINGS (0x0a)
        av_pairs[NTLMSSP_AV_CHANNEL_BINDINGS] = b'\x00' * 16
        
        # ============================================================
        
        # IMPORTANTE: Reconstruir serverName con el orden correcto
        serverName = av_pairs.getData()
        
        # Debug: verificar que serverName no esté vacío
        if len(serverName) < 20:
            raise Exception("ServerName/AV_PAIRS demasiado corto: {} bytes".format(len(serverName)))
